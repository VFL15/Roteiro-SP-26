            ...
            if (found && typeof found.evento === 'object') {
                    ...
                    evento.imagens.forEach((img, idx) => {
                        imagesHtml += `
                                     <a href="${img}" target="_blank" rel="noopener noreferrer" class="midia-item" data-img-index="${idx}"
                                         draggable="false"
                                         style="text-decoration:none; display:block; width:calc((100% - 10px) / 2); cursor:pointer; user-select:none; touch-action:pan-y; -webkit-user-select:none;">
                ...
                modalContent.innerHTML = `
                    ...
                    <div class="modal-tab-content" id="tab-midia" style="display: block;">
                        <button id="edit-images-btn" style="margin-bottom: 12px; padding: 6px 12px; background: #9F3132; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 12px;">✏️ Alterar posição das imagens</button>
                ...
                const toggleEditMode = (button) => {
                    ...
                    button.textContent = isEditingImages ? '✅ Pronto' : '✏️ Alterar posição das imagens';
